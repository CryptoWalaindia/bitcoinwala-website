...
const ContactModal: React.FC<ContactModalProps> = ({ open, onClose }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      trackModalOpen('contact') // Track modal open
    } else {
      setSubmitted(false)
  ...
  // Form submit handler - sends email and saves to database
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send email using EmailJS AND save to Supabase in parallel
      ...
      if (dbResult.status === 'rejected') {
        ...
        // Don't throw error - we still want to show success if email worked
      } else {
      ...
      if (emailResult.status === 'fulfilled' || dbResult.status === 'fulfilled') {
        setIsSubmitting(false)
        setSubmitted(true)
        trackContactFormSubmission(audience) // Track successful submission
      } else {
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div>
              <input
                ...
                inputMode="tel"
                pattern="[0-9+\-\s()]{6,20}"
                title="Please enter a valid phone number (6-20 characters, digits, spaces, +, -, or parentheses)"
                required
                className="w-full rounded-xl border border-[#1A1A1A] bg-black/25 px-4 py-3 text-sm placeholder-[#A0A0A0] focus:outline-none focus:ring-2 focus:ring-white/20"
              />
            </div>
